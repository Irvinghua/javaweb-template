...
本工程前端已**重构 UI**（浅色仪表盘风格：大留白、纯白卡片、统一圆角 + 柔和阴影、单一主色克制使用）。

> 📐 **新增或修改任何前端页面前，务必先阅读** 👉 [`jeecgboot-vue3/design.md`](jeecgboot-vue3/design.md)

它是本系统 UI 的**权威设计规范**，系统性定义了：

- **设计令牌（Design Tokens）**：品牌色（`--brand-500` = `#1A56FF`）、表面 / 文字 / 语义状态色、圆角 / 阴影 / 动效
- **字体排印 / 布局结构**：三层信息架构、侧边导航、卡片、子页内子导航
- **组件库**：按钮（高 38px）、表单控件、标签 pill、数据表格、分页、KPI 统计卡、分段切换、弹窗层、加载骨架
- **图标规范**

凡新建页面、用代码生成器 / AI 生成后做美化、或调整样式，都应**对照 `design.md` 的令牌与组件规范**，确保与现有系统视觉完全一致——而非随意自定义样式。
