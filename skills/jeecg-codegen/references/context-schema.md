...
> ⚠️ **对齐模板实际只消费一小部分字段**，多余字段（`extendParams`、`fieldRowNum`、`classType` 的字典类取值、`dictField`/`dictTable`/`dictText` 等）normalize 会兜底保留但**模板不读**，设了无效。真正驱动产物的是：
> - 顶层：`bussiPackage` `entityPackage` `entityName` `tableName` `description` `primaryKeyField` `originalColumns` `subTables`（一对多）
> - 列 `po`：`fieldName` `filedComment`(注意拼写) `fieldType`（决定控件）`fieldDbType`（决定 DB/Entity 类型）
> - `tableVo`：`ftlDescription` `searchFieldNum`（查询区展开字段数）
> - 子表 `sub`：`entityName` `ftlDescription` `tableName` `foreignKeys` `originalColumns`
>
> 字典/关联/上传/用户选择等控件一律**生成后 post-edit**（见 `dict-matching.md`、`post-edit-recipes.md`），不靠 ctx。
